...
![image-20230619215059320](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192150452.png)
...
+ 对临界资源的访问,必须互斥地进行,从逻辑上分为四个部分:
    + ![image-20230619214159407](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192141495.png)
    + 如图
        + 进入区
...
![image-20230619221910054](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192219136.png)
...
![image-20230619215554475](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192155552.png)
...
![image-20230619220016423](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192200523.png)
...
+ 违反忙则等待原则
    ...
    + 即在检查对方的$flag$后和切换自己的$flag$前之间有一段时间,结果都会检查通过,检查和修改操作不能一次性进行.
...
![image-20230619220604438](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192206518.png)
...
![image-20230619220910125](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192209204.png)
...
+ ![image-20230619222049281](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192220357.png)
...
![image-20230619222304154](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192223232.png)
...
![image-20230619223038627](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192230699.png)
...
![image-20230619233445826](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192334926.png)
...
![image-20230619230006210](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192300303.png)
...
![image-20230619232629363](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192326457.png)

![image-20230619232703369](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192327461.png)
...
![image-20230619234034513](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192340601.png)
...
4. 在“前操作”之后执行`V(S)`.
    + 必须保证“一前一后”执行的两个操作中**在前**的操作
5. 在“后操作”之前执行`P(S)`.
    + 必须保证“一前一后”执行的两个操作中**在后**的操作
    + 技巧口诀:**前$V$后$P$**

![image-20230619234808129](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192348230.png)
...
![image-20230619235155088](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192351183.png)
...
![image-20230619235433357](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192354443.png)
...
![image-20230619235341960](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192353047.png)
...
+ 往缓冲区放入/取走产品需要互斥.
+ 前驱图为
    + ![image-20230620212225669](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202122771.png)


所以设置三个变量,不能合并:

![image-20230620212404520](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202124715.png)

若调换$P$操作顺序会怎么样？(如下图,但这图是错的)

![image-20230620212621282](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202126378.png)
...
+ 前驱图为
    + ![image-20230620223739352](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202237484.png)
...
![image-20230620214945413](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202149607.png)
...
+ ![image-20230620224333967](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202243098.png)
...
![image-20230620225046284](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202250377.png)

**【实现代码】:**

![image-20230620225515739](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202255856.png)
...
+ 同步关系,前$V$后$P$
    ...
    + 既然各个读进程需要同时访问,而读进程与写进程又必须互斥访问,那么可以让第一个访问文件的读进程“加锁”,让最后一个访问完文件的读进程“解锁”
    + 可以设置一个整数变量`count`来记录当前有几个读进程在访问文件,同时保持读进程互斥,避免多个读进程同时操作导致计数错误.

**【实现代码】:**

![image-20230620231207592](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202312709.png)

+ 仅由第一个读进程负责加锁,最后一个读进程负责解锁
+ 但是当两个读进程并发执行,则$count=:0$时两个进程也许都能满足f条件,都会执行$P(w)$,从而使第二个读进程阻塞的情况
    + 解决方法:根本原因在于对`count`变量的检查和赋值无法一气呵成,因此可以设置另一个互斥信号量来保证各读进程对`count`的访问是互斥的

![image-20230620231448626](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202314756.png)
...
![image-20230620232207086](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202322206.png)
...
![image-20230620234729631](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202347729.png)
...
![image-20230621203347854](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306212033009.png)
...
![image-20230621202517743](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306212025950.png)
...
+ 该策略的缺点:
    ...
    2. 释放已获得的资源可能造成前一阶段工作的失效
        + 因此这种方法一般只适用于易保存和恢复状态的资源,如$CPU$,而不能用于打印机.
...
+ 原理分析
    + 一个进程只有已占有小编号的资源时,才有资格申请更 大编号的资源
...
+ 以上代码中使用的数据结构
    ...
    + ![image-20230621213022254](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306212130368.png)
...
![image-20230621213602131](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306212136238.png)
