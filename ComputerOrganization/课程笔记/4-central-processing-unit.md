...
>
>   按照流水线设计原则，每个流水段的长度为200ps，所以每条指令的执行时间为Ins，反而比串行执行时增加了 250ps。
>
>   假设某程序中有N条指令，单周期处理机所用的时间为Nx750ps，而流水线处理机所用的时间为（N+4）x200ps。
...
+   图5.19中的控制信号(虚线表示)如表5.3所示。控制信号的来源并不一致，如IF段的分支转跳信号BranchTaken来源于EX段，ID段的RegWrite信号来源于WB段，其他控制信号通过控制器产生
+   由ID段负责译码生成控制信号，并分别在随后的各个时钟周期内使用。
