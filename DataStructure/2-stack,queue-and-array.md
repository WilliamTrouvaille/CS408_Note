...
+   $\cdots$
+   第$i-1$行有$n-(i-2)$个元素
    +   为方便推导可以先得出==第$i$行一共有$n-(i-1)$个元素==
    +   再令上式中$i=i-1$

+   第$i$行$a_{ij}$之前有$j-i$个元素。
    +   考虑$a_{ij}$所在行第$i$行一共需要存储$n+1-i$
        +   如上面标黄处
...
+ 当$i\leqslant j$时，即==上三角区域与主对角线元素==下标$k=\dfrac{(i-1)(2n-i+2)}{2}+(j-i)$。
...
+   矩阵中非零元素的个数$t$，相对矩阵元素的个数$s$来说非常少，即$s\gg t$的矩阵称为**稀疏矩阵**。 
    +   例如，一个矩阵的阶为$100\times100$，该矩阵中只有少于$100$个非零元素。 
+   三元组法:
