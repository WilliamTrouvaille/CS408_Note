...
![编码示意图](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202307261206515.png)
...
![归零编码](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202307261207292.png)

归零编码：折$1$平$0$，信号电平在一个码元内都要恢复到$0$。信道使用率低，基本上都是无使用。

![反向不归零编码](https://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202307261207309.png)
...
4. 正交振幅调制（$QAM$）。在频率相同的前提下，将$ASK$与$PSK$结合起来，形成叠加信号。设波特率为$B$，采用$m$个相位，每个相位有$n$种振幅，码元类型=相位数×振幅数。则该$QAM$技术的数据传输速率$R$为$R=B\log_2(mn)$，即为波特率×$\log_2\,$(相位数×每相位振幅数)。
