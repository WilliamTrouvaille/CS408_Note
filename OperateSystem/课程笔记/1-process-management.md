...
![image-20230611112836892](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306111128940.png)
...
+   父进程和子进程==可以共享一部分资源==
    +   但是==不能共享虚拟地址空间==，因为是逻辑地址且其有不同的$PCB$用于区分进程.
...
+ 共享存储：分配一个可以共同使用的共享空间，进程对其的使用必须是互斥的（使用同步互斥工具如$PV$操作）.
    + 基于数据结构的实现：共享空间只能放固定的数据结构如数组等。速度慢，限制多，是低级通信方式。
    
    + 基于存储区的实现：内存中划出一块共享存储区，数据的形式、存放位置都由进程控制而非操作系统。速度更快，限制少，是高级通信方式。
...
+ 管道通信：是消息传递的特殊方式，指用于链接各自一个的读写进程的一个共享文件，又名$pipe$文件，其实就是内存中开辟一个大小**固定**的缓冲区.  
    + 只能使用半双工的通信，**同一时间段**内只能**单向**传输，若要**双向同时**通信则必须设置两个管道.
        + 一个普通管道只允许**单向通信**
...
![image-20230612110559956](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306121105999.png)

## 处理机调度

![image-20230618210522721](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306182105842.png)

![image-20230823115550999](https://raw.githubusercontent.com/WilliamTrouvaille/image_hosting/main/CS408_NOTE/20251023153207906.png)
...
![image-20230823141712141](https://raw.githubusercontent.com/WilliamTrouvaille/image_hosting/main/CS408_NOTE/20251023153212606.png)
...
+ 剥夺调度方式，又称抢占方式：
    ...
    + 可以优先处理更紧急的进程，也可实现让各进程按时间片轮流执行的功能（通过**时钟中断**）.
    + 适合于分时操作系统、实时操作系统.
...
![image-20230823144304672](https://raw.githubusercontent.com/WilliamTrouvaille/image_hosting/main/CS408_NOTE/20251023153215680.png)
...
![image-20230823144544372](https://raw.githubusercontent.com/WilliamTrouvaille/image_hosting/main/CS408_NOTE/20251023153218376.png)
...
![同步与互斥](https://raw.githubusercontent.com/WilliamTrouvaille/image_hosting/main/CS408_NOTE/20251023153221988.png)

![image-20230828113344964](https://raw.githubusercontent.com/WilliamTrouvaille/image_hosting/main/CS408_NOTE/20251023153225940.png)
...
![image-20230619220604438](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192206518.png)
...
+ 每个临界资源都有一个共享布尔变量`lock`，`true`表示已上锁被占用，`false`表示未上锁空闲，是初值.
...
1. 分析什么地方需要实现“同步关系”
    + 即必须保证“一前一后”执行的两个操作（或两句代码）.如先$P_1$后$P_2$.
2. 设置同步信号量`S`，初始为$0$.
3. 同步信号量为整数，释放加一，占用减一.
4. 在“前操作”**之后**执行`V(S)`.
    + ==必须保证“一前一后”执行的两个操作中**在前**的操作已经释放好资源，在完成操作**之后释放资源**==
5. 在“后操作”**之前**执行`P(S)`.
    + ==必须保证“一前一后”执行的两个操作中**在后**的操作，必须保证在执行操作前能够拥有全部资源，故应该**在开始操作之前检查并申请资源**==
    + 技巧口诀：**前$V$后$P$**

![image-20230619234808129](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306192348230.png)
...
+ 若先进行进程$P_2$：
    ...
    + 由于此时有进程在该信号量对应的阻塞队列中，因此会在$V$操作中执行`wakeup`原语，唤醒$P_2$进程
    + 这样$P_2$就可以继续执行代码$4$了.
...
+ $value>0$表示某类可用资源的数量，每次$P$操作，意味着请求分配一个单位的资源.
+ $value\ge0$表示某类资源已经没有，或者说还有因请求该资源而被阻塞的进程.
+ $value\le0$时的绝对值，表示等待进程数目.
...
+ 有时候是消费者需要等待生产者生产，有时候是生产者要等待消费者消费，这是两个不同的“一前一后问题”，因此也需要设置两个同步信号量
    + 前生产者$V$后消费者$P$的关系就是`full`，前消费者$V$后生产者$P$的关系就是`empty`.
...
+ 前驱图为
    ...
    ![image-20230620212225669](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202122771.png)


所以设置三个变量，不能合并：

![image-20230620212404520](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202124715.png)

若调换$P$操作顺序会怎么样？(如下图，但这图是错的)

![image-20230620212621282](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202126378.png)

+ 若此时缓冲区内已经放满产品
    + 即`empty=0`，`full=n`.

+ 则生产者进程执行`P(mutex)`使`mutex`变为`0`上锁，再执行`P(empty)`
    + 由于已没有空闲缓冲区，因此生产者被阻塞.

+ 生产者还没释放对临界资源的“锁”，因此消费者也被阻塞.
    + 消费者进程执行`P(mutex)`，由于`mutex`为`0`

+ 这就造成了生产者等待消费者对产品消费来释放空闲缓冲区，而消费者又等待生产者解锁临界区的情况，生产者和消费者**循环等待被对方唤醒**，出现“**死锁**”.
+ 同样的，若缓冲区中没有产品，即`full=0`，`empty=n`，按先消费者后生产者的顺序执行也会发生死锁.
+ 因此，==实现同步的$P$操作一定要在实现互斥的$P$操作之前.可以理解为要先拿到这个空间再上锁==，若没有就拿到就上锁，那么其他进程也用不了这个空间.

**【注意】：**

+ 实现同步的$empty$和$full$($P$操作)必然在实现互斥的$mutex$的$P$操作之前
+ 如果先上锁再获取可能会出现死锁现象
+ 由于$V$操作是释放不会导致进程阻塞，所以两个$V$操作可以交换顺序.
+ 上述代码中生产产品和使用产品的代码理论上可以放进$PV$操作之间，但是由于**会对系统性能产生影响**所以一般不放进去
+ ==互斥信号量初值为`1>0`，同步信号量初值为`0`==
    + 可以这么理解
        + 两个信号量的实质都是某资源的可使用数量
        + 互斥信号量表示互斥资源的数量，一般表示存储资源的空间等，这种资源一开始是存在的，初值为`1`
            + 所谓互斥使用某临界资源，是指在同一时间段只允许一个进程使用此资源，所以互斥信号量
                的初值都为`1`

        + 本问题中，同步信号量表示同步资源的数量，一般表示需要生产和消耗的资源等，这种资源一开始是不存在需要生产之后才会后的，所以初值为`0`
            + 同步信号量的初值应根据具体情况来确定，由用户决定。
                + 若期望的消息尚未产生，则对应的初值应设为0
...
+ 在分析同步问题特别是多生产者多消费者问题的时候不能从单个进程行为的角度来分析，要把“一前一后”发生的事看做是两种“事件”的前后关系.
...
+ 正确的分析方法应该从“事件”的角度来考虑，我们可以把上述四对“进程行为的前后关系”抽象为一对“事件的前后关系”
    ...
    + 将苹果和橘子都归为水果，而互斥的是盘子而不是不同的水果，同步的是先盘子再水果，即
        + 前事件：盘子变空事件
            + `盘子变空事件`既可由儿子引发，也可由女儿引发
        + 后事件：放入水果事件
            + `放入水果事件`既可能是父亲执行，也可能是母亲执行
    + 这样的话，就可以用一个同步信号量`plate=1`和水果信号量解决问题了.
...
+ 前驱图为
    ...
    ![image-20230620223739352](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202237484.png)
...
+ 对于同步关系
    + 同步信号量苹果设置为`apple=0`
        + 表示盘子中是否有苹果，初值为`0`表示盘子为空，不许取，`apple = 1`表示可以取
    + 同步信号量橘子设置为`orange=0`
        + 表示盘子中是否有橘子， 初值为`0`表示盘子为空，不许取，`orange = 1`表示可以取
    + 同步信号量盘子设置`plate=1`(盘子中还可以放多少个水果)
        + 也可以设置为`0`，只不过后面的处理不同(盘子中有多少个水果)

**【实现代码】：**

![image-20230620214945413](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202149607.png)
...
+ 对于父母
    1. 先准备一个苹果/橘子
    2. 放苹果之前，先判断盘子里是否为空
        + 对盘子进行`P`操作，检查盘子中还可以放多少个水果
    3. 如果盘子为空进行加锁
    4. 把苹果/橘子放进去
    5. 解锁
    6. 对苹果/橘子进行`V`操作，数量+1
+ 对于子女
    1. 拿苹果/橘子之前，先判断盘子里有没有苹果/橘子
        + 对苹果/橘子进行`P`操作，若没有苹果/橘子，自己被阻塞
    2. 如果有就锁定盘子
    3. 取出苹果/橘子
    4. 解锁
    5. 告诉父母，盘子为空了
...
+ 假如刚开始是父亲进程先上处理机运行，则
    ...
    4. 女儿进程被唤醒，其他进程即使运行也都会阻塞，暂时不可能访问临界资源（盘子）
...
+ 即本题中，不设置专门的互斥信号量`mutex`时也不会出现多个进程同时访问盘子的现象.

+ 原因在于本题中的缓冲区大小为`1`，在任何时刻，`apple`、`orange`、`plate`三个同步信号量中最多只有一个是$1$.因此在任何时刻，最多只有一个进程的$P$操作不会被阻塞，并顺利地进入临界区，此时互斥关系全部变成同步关系.

    + 需要注意的是，不一定所有缓冲区大小为`1`的情况都可以不用设置专门的互斥信号量`mutex`，需要结合具体情况具体分析

+ 本题中，而如果缓冲区大于$1$，则父母两个可以同时访问临界区，可能导致数据覆盖，所以必须使用互斥信号量.
...
+ 值得吸取的精华是：
    + “轮流让各个吸烟者吸烟”必然需要“轮流的在桌上放上组合一、二、三”，注意体会用一个整型变量`ⅰ`是如何实现这个“轮流”过程的.
...
+ 供应者进程无限地提供三种材料，供应者每次将两种材料放桌子上，拥有剩下那种材料的抽烟者卷一根烟并抽掉它，并给供应者进程一个信号告诉完成了，供应者就会放另外两种材料再桌上，这个过程一直重复（让三个抽烟者**轮流地**抽烟）

    ![image-20230620224333967](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202243098.png)



**【关系分析】：**

+ 桌子可以抽象为容量为**1**的缓冲区要互斥访问
    + 由于供应者提供的三种材料组合都只存在一个抽烟者需求，所以应该将三种材料组合分别视为一种整体
+ 同步关系（从事件的角度来分析）：
    + 桌上有组合一$\to$第一个抽烟者取走东西
    + 桌上有组合二$\to$第二个抽烟者取走东西
    + 桌上有组合三$\to$第三个抽烟者取走东西
...
+ 互斥关系，互斥访问
    + 分析：缓冲区大小为1，且满足同一时刻四个同步信号量中至多有一个的值为1
    + 经过分析，本题可以不用设置互斥信号量
+ 同步关系，前$V$后$P$
    + 桌子上组合一，二，三的数量分别设置信号量`offer1=0`，`offer2=0`，`offer3=0`
    + 吸烟完成设置信号量`finish=0`

![image-20230620225046284](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202250377.png)

**【实现代码】：**

![image-20230620225515739](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202255856.png)
...
+ 可以用`count`的值来判断当前进入的进程是否是第一个/最后一个读进程，从而做出不同的处理.
+ 另外，对`count`变量的检查和赋值不能一气呵成导致了一些错误，如果需要实现“一气呵成”，自然应该想到用互斥信号量对`count`进行$PV$操作.
+ 绝大多数的考研$PV$操作大题，在同步方面，都可以用之前介绍的几种生产者消费者问题的思想来解决，如果遇到更复杂的涉及互斥方面的问题，可以尝试能否利用读者写者问题的这几个思想来解决.
...
+ 因此要求：
    1. 允许多个读者可以同时对文件执行读操作
        + 此处的读进程与之前提到的消费者进程不同，==读者进程在读数据后并不会像消费者进程一样将数据清空，并不会改变数据==
        + 因此允许多个读者同时访问共享数据
    1. 只允许一个写者往文件中写信息
    1. 任一写者在完成写操作之前不允许其他读者或写者工作写
...
+ 互斥关系，互斥访问
    + 读进程与读进程不存在互斥问题，不需要设置互斥信号量
    + 写者进程和任何进程都互斥，设置一个互斥信号量`rw`，在写者访问共享文件前后分别执行$P$、$V$操作.
    + 读者进程和写者进程也要互斥，因此读者访问共享文件前后也要对`rw`执行$P$、$V$操作.
+ 同步关系，前$V$后$P$
    + `P(rw)`和`V(rw)`其实就是对共享文件的“加锁”和“解锁”.
    + 既然各个读进程需要同时访问，而读进程与写进程又必须互斥访问，那么可以让==第一个访问文件的读进程“加锁”，让最后一个访问完文件的读进程“解锁”==
    + 可以设置一个整数变量`count`来记录当前有几个读进程在访问文件，同时保持读进程互斥，避免多个读进程同时操作导致计数错误.

**【实现代码】：**

![image-20230620231207592](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202312709.png)

+ 仅由第一个读进程负责加锁，最后一个读进程负责解锁
+ 但是当两个读进程并发执行，则$count=0$时两个进程也许都能满足if条件，都会执行$P(w)$，从而使第二个读进程阻塞的情况
    + 解决方法：根本原因在于对`count`变量的检查和赋值无法一气呵成，因此可以设置另一个互斥信号量来保证各读进程对`count`的访问是互斥的

![image-20230620231448626](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202314756.png)

+ **存在饥饿现象**
    + 读进程是优先的，只要存在读进程运行写进程就永远无法写，导致写进程可能饥饿.
+ 解决方法：先写后读，即设置一种同步关系
    + 再添加一个信号量`w=1`并进行一对$PV$操作
    + 尽量使写进程优先
    + 当读进程读共享文件时，有写进程访问就立刻禁止后续读进程的请求
    + 当前所有读进程都执行完毕后立刻执行写进程
    + 只有无写进程执行再执行读进程

![image-20230620232207086](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202322206.png)
...
+ 哲学家进餐问题的关键在于解决进程死锁.
+ 这些进程之间**只存在互斥关系**，但是与之前接触到的互斥关系不同的是，每个进程都需要同时持有两个临界资源，因此就有“死锁”问题的隐患.
+ 如果在考试中遇到了**一个进程需要同时持有多个临界资源**的情况，应该参考哲学家问题的三种思想，分析题中给出的进程之间是否会发生循环等待，是否会发生**死锁**.
...
+ 定义互斥信号量数组`chopstick[5]={1，1，1，1，1}`用于实现对$5$个筷子的互斥访问.并对哲学家按$0\sim4$编号
    + 编号为$i$的哲学家左边的筷子编号为$i$，右边的筷子编号为$(i+1)\%5$.

+ 为了防止死锁：
    1. 可以对哲学家进程施加一些限制条件，比如最多允许四个哲学家同时进餐
        + 这样可以保证至少有一个哲学家是可以拿到左右两只筷子的.
        + 通过限制并发进餐的数量，破坏了死锁发生的循环等待条件
    2. 要求奇数号哲学家先拿左边的筷子，然后再拿右边的筷子，而偶数号哲学家刚好相反.用这种方法可以保证如果相邻的两个奇偶号哲学家都想吃饭，
        + 这种情况只会有其中一个可以拿起第一只筷子，另一个会直接阻塞
        + 避免了占有一支后再等待另一只的情况
        + 通过规定不同编号哲学家的筷子拿取顺序，破坏了死锁发生的循环等待条件
    3. 同时仅允许当一个哲学家左右两支筷子都可用时才允许他抓起筷子
        + 重点是**一个时刻只有一个哲学家才能尝试拿筷子**
        + “两支筷子都可用时才允许抓起筷子”实际上不太准确，因为取左边筷子和取右边筷子两个事件无法一气呵成
            + 一种比较准确的说法应该是：各哲学家拿筷子这件事必须互斥的执行.
            + 这就保证了即使一个哲学家在拿筷子拿到一半时被阻塞，也不会有别的哲学家会继续尝试拿筷子
            + 以便当前正在吃饭的哲学家放下筷子后，被阻塞的哲学家就可以获得等待的筷子了
            + 通过要求同时可用的筷子，破坏了死锁发生的请求和保持条件
...
>   Chandy/Misra解法
>
>   1984年，曼尼·钱迪和贾亚达夫·米斯拉提出了哲学家就餐问题的另一个解法，允许任意的用户争用任意数量的资源。与资源分级解法不同的是，这里编号可以是任意的。
>
...
2. 一个进程只有通过调用管程内的过程才能进入管程访问**共享数据**
    + 即想要修改管程中定义的共享数据结构只能通过管程内部定义的函数修改
        + ~~类似于实体类中那一大堆的$setter$~~
            + ~~虽然有`lombok.@Data`不用自己写(~~
        + 类似于个锤子，不一样的
        + ==这里只能访问共享数据，无法访问私有数据==
...
![image-20230620234729631](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306202347729.png)

+ 生产者进程只需要调用管程内部的`insert`方法就不用管了，剩下的互斥和同步问题交给管程解决
    + 由编译器负责实现各进程**互斥**地进入管程中的过程
    + 在管程中设置**条件变量**和等待/唤醒操作以解决**同步**问题
...
5. 可在管程中设置条件变量及等待/唤醒操作以解决同步问题.可以让一个进程或线程在条件变量上等待
    + 此时，该进程应先释放管程的使用权，也就是让出“入口”
...
`synchronized`关键字提供了以下功能：

1. 互斥性：类似于互斥锁，在同一时间只有一个线程可以进入临界区，防止多个线程同时访问共享资源，避免数据竞争和不一致的问题.
2. 重入性：允许同一个线程多次进入临界区.类似于管程中的进程重入，允许一个线程在持有锁的情况下再次进入同一个`synchronized`临界区，避免死锁.
3. 可见性：保证线程对共享资源的修改对其他线程可见.当一个线程离开`synchronized`临界区时，它会释放锁，并将对共享资源的修改刷新到主内存，以便其他线程能够读取最新的值.
4. 等待和通知：通过内置的监视器（monitor）机制，可以使用`synchronized`关键字配合`wait()`和`notify()`方法实现等待和通知的功能，类似于管程中的条件变量.线程可以在`synchronized`临界区中调用`wait()`方法等待某个条件的满足，其他线程可以调用`notify()`方法唤醒等待的线程.

## 死锁

### 死锁的概念

![image-20230621203347854](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306212033009.png)
...
![image-20230621202517743](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306212025950.png)
...
3. **请求和保持条件**：进程已经保持了至少一个资源，但又提出了新的资源请求，而该资源又被其他进程占有
    ...
    + 此时请求进程被阻塞，但又对自己己有的资源保持不放.

4. **循环等待条件**：存在一种进程资源的循环等待链，链中的每一个进程己获得的资源同时被下一个进程所请求.

    + **发生死锁时一定会有循环等待，但是发生循环等待时未必死锁**
        + 循环等待是死锁的必要不充分条件
...
2. 进程推进顺序非法
    ...
    + 例如
        ...
        + 并发执行的进程$P1$、$P2$分别申请并占有了资源$R1$、$R2$
        + 之后进程$P1$又紧接着申请资源$R2$，而进程$P2$又申请资源$R1$
        + 两者会因为申请的资源被对方占有而阻塞
...
3. 信号量的使用不当也会造成死锁
    ...
    + 例如生产者-消费者问题中，如果==实现互斥的$P$操作在实现同步的$P$操作之前，就有可能导致死锁==
...
+ 例如$SPOOLing$技术.
    ...
    + 比如使用$SPOOLing$技术将打印机改造为共享设备，将多个进程的请求合并为一个输出进程.
+ 该策略的缺点
...
+ 可以考虑的方案：
    ...
    1. 当某个进程请求新的资源得不到满足时，它必须立即释放保持的所有资源，待以后需要时再重新申请
        + 也就是说，即使某些资源尚未使用完，也需要主动释放，从而破坏了不可剥夺条件.

    2. 当某个进程需要的资源被其他进程所占有的时候，可以由操作系统协助，将想要的资源强行剥夺

        + 这种方式一般需要考虑各进程的优先级

        + 比如剥夺调度方式，就是将处理机资源强行剥夺给优先级更高的进程使用

+ 该策略的缺点：

    1. 实现起来比较复杂.
    2. 释放已获得的资源可能造成前一阶段工作的失效
        + 因此这种方法一般只适用于易保存和恢复状态的资源，如$CPU$，而==不能用于打印机.==
...
+ 可以采用**静态分配方法**
    ...
    + 在它的资源未满足前，不让它投入运行
    + 一旦投入运行，这些资源就一直归它所有，该进程就不会再请求别的任何资源

+ 该策略实现起来简单，但也有明显的缺点：
    1. 资源利用率极低
        + 有些资源可能只需要用很短的时间，因此如果进程的整个运行期间都一直保持着所有资源，就会造成严重的资源浪费
    2. 该策略也有可能导致某些进程饥饿.
...
+ 原理分析
    ...
    + 即在任何一个时刻，总有一个进程拥有的资源编号是最大的，那这个进程申请之后的资源必然畅通无阻，因此，不可能出现所有进程都阻塞的死锁现象
...
+ 如果系统进入不安全状态，就可能发生死锁
    + 处于不安全状态未必已经发生了死锁，但发生死锁时一定是在不安全状态
...
+ 做题时还有一个更快的方法得到安全序列：将剩余资源数与最多还需要对比，满足条件的进程全部加入安全序列，然后把归还的资源相加，进行下一轮的比较.

+ 使用代码实现：

    1. 初始化：

        1. 假设系统中有$n$个进程，$m$种资源.

            + 可以用一个$n\times m$的分配矩阵$Allocation$表示对所有进程的资源分配情况

            + `Allocation[i，j]=K`表示进程$P_i$当前已经分片到$R_j$类资源的数目为$K$.

        2. 用一个长度为$m$的一维数组 $Available$表示当前系统中还有多少可用资源.

            + `Available[j]=K`表示系统中有$R_j$类资源$K$个.

        3. 用一个$n\times m$的矩阵$Max$表示所有进程对各种资源的最大需求数

            + `Max[i，j]=K`表示进程$P_i$最多需要$K$个资源$R_j$

    2. $Max-Allocation=Need[i，j]$矩阵，表示各进程最多还需要多少各类资源

    3. 某进程$P_i$向系统申请资源，使用一个长度为$m$的一维数组$Request_i$表示本次申请的各种资源量

    4. 可用**银行家算法**预判本次分配是否会导致系统进入不安全状态：

        1. 如果$Request_i[j] \le Need[i，j]$（$0\leqslant j\leqslant m$）则转向步骤`4-2`

            + 否则它所需要的资源数已超过它所宣布的最大值，认为出错

        2. 如果$Request_i[j] \le Available[i]$（$0\leqslant j\leqslant m$），便转向步骤`4-3，`

            + 否则表示尚无足够资源，$P_i$必须等待.

        3. 系统试探着把资源分配给进程$P_i$，并修改相应的数据（并非真的分配，修改数值只是为了做预判）.

            + $Available[i] -=  Request_i[j]$

            + $Allocation[i，j] +=  Request_i[j]$

            + $Need[i，j] -= Request_i[j]$

        4. 操作系统执行**安全性算法**，检查此次资源分配后，系统是否处于安全状态
            ...
            + **安全性算法**，设置工作向量$Work$，有$m$个元素，表示系统中的剩余可用资源数目.在执行安全性算法开始时，`Work = Available`.
                ...
                2. 从$Need$矩阵中找出符合下面条件的行
                    ...
                    找到后，把对应的进程加入安全序列；若找不到，则执行步骤四.

                3. 进程$P_i$进入安全序列后，可顺利执行，直至完成，并释放分配给它的资源，

                    + 应执行
                        `Work = Work + Allocation[i]`
                    + 其中`Allocation[i]`表示进程$P_i$代表的在$Allocation$矩阵中对应的行

                    返回步骤`4-4-2`.若此时安全序列中已有所有进程，则系统处于安全状态，否则系统处于不安全状态.

+ 以上代码中使用的数据结构

    ![image-20230621213022254](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306212130368.png)
    
    + 长度为$m$的一维数组 $Available$表示当前系统中还有多少**可用资源**
    + $n\times m$的矩阵$Max$表示所有进程对各种资源的**最大需求数**
    + $n\times m$的矩阵$Allocation$表示对所有进程的**资源分配情况**
    + $n\times m$的矩阵$Need[i，j]=Max-Allocation$矩阵，表示各进程**还需要多少各类资源**
    + 长度为$m$的一维数组$Request_i$表示进程$P_i$**本次申请的各种资源量**
    + 长度为$m$的一维数组向量$Work$表示**执行安全性算法时系统中的剩余可用资源数目**
...
1. 使用**某种数据结构**来保存资源的请求和分配信息
2. 提供一种算法，利用上述数据结构来检测系统是否已进入死锁状态

检测数据结构就是**资源分配图**：

![image-20230621213602131](http://trouvaille-oss.oss-cn-beijing.aliyuncs.com/picList/202306212136238.png)
...
1.   资源分配图中出现环路，满足了循环等待的必要条件，可能（不一定）发生死锁
...
2. 撤销进程法（或称终止进程法）
    ...
    + 这种方式的优点是实现简单
    + 但所付出的代价可能会很大.因为有些进程可能已经运行了很长时间，已经接近结束了，一旦被终止可谓功亏一篑，以后还得从头再来.
3. 进程回退法
